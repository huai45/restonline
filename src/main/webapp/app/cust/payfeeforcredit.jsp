...
$(document).ready(function(){
    ...
    $("#submitBtn").live("click",function(){
        ...
        if (recvfee == "") {
            $("#money_err").text('*请输入交费金额');
			$("#recv_fee_input")[0].focus();
			return false;
		}
		if (isNaN(recvfee)) {
			$("#money_err").text('*交费金额只能是数字');
			$("#recv_fee_input")[0].focus();
			return false;
		}
		if ( parseInt(recvfee)==0 ) {
			$("#money_err").text('*交费金额不能为0');
			$("#recv_fee_input")[0].focus();
			return false;
		}
		ajax_flag = 1;
		$.post("/cust/payfeeForCredit.html", {
		        user_id : user_id,
...
<div id="center" data-options="region:'center',border:false,style:{borderWidth:0}" style="padding:0px;background:#FFF;">
    <div style="height:41px;line-height:41px;width:90%;font-size:16px;border-bottom:solid #FFF 1px;margin:0px 40px 10px 0px;border-bottom:solid 1px #DDD;">
	    <div style="float:left;margin-left:20px;" >挂账用户交费冲正</div>
    </div>
    <div style="padding-left:20px;">
        <div style="line-height:30px;font-size:16px;margin-top:30px;">请输入交费金额：</div>
