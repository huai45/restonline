...
String sql = " select substring(oper_time,1,10) oper_date , ROUND(sum(price*(count-back_count)),2) count, ROUND(sum(price*(count-back_count-free_count)*pay_rate/100),2) real_count "+ 
  " from th_bill_item a where a.rest_id = ? and oper_time >= ? and oper_time <= ? and oper_staff_name = ? group by oper_date order by oper_date DESC ; ";
ut.log(" sql :" +sql);
List details = new ArrayList();
if(!start_date.equals("")&&!end_date.equals("")){
	details = jdbcTemplate.queryForList(sql,new Object[]{rest_id,start_date+" 00:00:00",end_date+" 23:59:59",staff_name});
}
List staffList = jdbcTemplate.queryForList(" select user_id,username from td_phone_user where rest_id = ? order by user_id ",
    new Object[]{rest_id} );
